...
class TriangleModel:
    ...
    def create_from_charts(self, triangle_params, spatial_lr_scale: float, opacity: float, init_size: float, nb_points: int, set_sigma: float, no_dome: bool):
        """Create triangle model from triangle parameters obtained from charts data.
        
        Args:
            triangle_params (dict): Dictionary containing triangle parameters from get_triangle_parameters_from_charts_data
            spatial_lr_scale (float): Spatial learning rate scale
            opacity (float): Initial opacity value
            init_size (float): Initial size parameter
            nb_points (int): Number of points per triangle
            set_sigma (float): Sigma parameter for triangle rendering
            no_dome (bool): Whether to use dome or not
        """
        # Extract triangle vertices and colors from the parameters
        if 'face_triangles' in triangle_params:
            # Use the original mesh face triangles
            triangles_reshaped = triangle_params['face_triangles']  # (n_faces, 3, 3)
        else:
            # Use triangle vertices (this would need to be reshaped properly)
            triangle_vertices = triangle_params['triangle_vertices']  # (N, 3)
            # Reshape to (N/3, 3, 3) assuming each triangle has 3 vertices
            triangles_reshaped = triangle_vertices.reshape(-1, 3, 3)
        
        triangle_colors = triangle_params.get('colors', None)  # (N, 3) or None
        
        # Set up model parameters
        self.spatial_lr_scale = spatial_lr_scale
        self.nb_points = nb_points
        
        # Convert colors to spherical harmonics if available
        if triangle_colors is not None:
            # If colors are per-vertex, we need to take the first vertex color for each triangle
            if triangle_colors.shape[0] == triangles_reshaped.shape[0] * 3:
                # Colors are per-vertex, take first vertex color for each triangle
                triangle_colors = triangle_colors[::3]  # Take every 3rd color (first vertex of each triangle)
            elif triangle_colors.shape[0] != triangles_reshaped.shape[0]:
                # Mismatch in dimensions, use default colors
                triangle_colors = None
        
        if triangle_colors is not None:
            fused_color = RGB2SH(triangle_colors)
        else:
            # Use default colors if not available
            fused_color = RGB2SH(torch.ones((triangles_reshaped.shape[0], 3), device='cuda') * 0.5)
        ...
        number_of_triangles = triangles_reshaped.shape[0]
        
        # Set up opacities and sigmas
        opacities = inverse_sigmoid(opacity * torch.ones((number_of_triangles, 1), dtype=torch.float, device="cuda"))
        sigmas = self.inverse_exponential_activation(torch.ones((number_of_triangles, 1), dtype=torch.float, device="cuda") * set_sigma)
        
        # Set up triangle-specific parameters
        tensor_num_points_per_triangle = torch.tensor([3 for i in range(number_of_triangles)], dtype=torch.int, device='cuda:0')
        cumsum_of_points_per_triangle = torch.cumsum(torch.nn.functional.pad(tensor_num_points_per_triangle, (1,0), value=0), 0, dtype=torch.int)[:-1]
        ...
        self._triangles_points = nn.Parameter(triangles_reshaped.to('cuda').requires_grad_(True))
        self._sigma = nn.Parameter(sigmas.requires_grad_(True))
        self._num_points_per_triangle = tensor_num_points_per_triangle
        self._cumsum_of_points_per_triangle = cumsum_of_points_per_triangle
        self._number_of_points = number_of_triangles
        
        # Initialize other parameters
        self.max_scaling = torch.zeros((number_of_triangles), dtype=torch.float, device="cuda")
        self.max_radii2D = torch.zeros((number_of_triangles), dtype=torch.float, device="cuda")
        self.max_density_factor = torch.zeros((number_of_triangles), dtype=torch.float, device="cuda")
        self._mask = nn.Parameter(torch.ones((number_of_triangles, 1), device="cuda").requires_grad_(True))
        self.triangle_area = torch.zeros((number_of_triangles), dtype=torch.float, device="cuda")
        self.image_size = torch.zeros((number_of_triangles), dtype=torch.float, device="cuda")
        self.importance_score = torch.zeros((number_of_triangles), dtype=torch.float, device="cuda")
    
    def create_from_ply(self, mvs_mesh_path, spatial_lr_scale : float, opacity : float, init_size : float, nb_points: int, set_sigma : float, no_dome: bool):
        # verts, faces = load_ply(mvs_mesh_path)
        mesh = o3d.io.read_triangle_mesh(mvs_mesh_path)
        ...
        points_colors = np.asarray(mesh.vertex_colors)

        triangles = points[faces, :]
        triangles_colors = points_colors[faces, :]

        fused_color = RGB2SH(torch.tensor(triangles_colors).float().cuda()[:, 0, :])
        features = torch.zeros((fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2)).float().cuda()
        features[:, :3, 0 ] = fused_color
        features[:, 3:, 1:] = 0.0

        number_of_points = triangles.shape[0]
        ...
        self._features_rest = nn.Parameter(features[:,:,1:].transpose(1, 2).contiguous().requires_grad_(True))
        self._opacity = nn.Parameter(opacities.requires_grad_(True))
        tensor_num_points_per_triangle = torch.tensor([3 for i in range(number_of_points)], dtype=torch.int, device='cuda:0')
        cumsum_of_points_per_triangle = torch.cumsum(torch.nn.functional.pad(tensor_num_points_per_triangle, (1,0), value=0), 0, dtype=torch.int)[:-1]
  
        self._triangles_points = nn.Parameter(torch.tensor(triangles).to('cuda').float().requires_grad_(True))
        self._sigma = nn.Parameter(sigmas.requires_grad_(True))
        self._num_points_per_triangle = tensor_num_points_per_triangle
        self._cumsum_of_points_per_triangle = cumsum_of_points_per_triangle
        self._number_of_points = number_of_points
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, opacity : float, init_size : float, nb_points: int, set_sigma : float, no_dome: bool):
        ...
        fused_point_cloud = torch.tensor(np.asarray(total_points)).float().cuda() # [N, 3]
        ...
        points_per_triangle = generate_triangles_in_chunks(x, y, z, radii, nb_points) #[N, 3, 3]
        ...
        ## Does num_points_per_triangle = fused_point_cloud.shape[0] = number_of_points ? -> Yes
    ...
    def add_new_gs(self, cap_max, oddGroup=True, dead_mask=None):
        ...
        probs[dead_mask] = 0
        # Check if all elements in dead_mask are True
